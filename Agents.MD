...
- Photography uploads now store variants under `photography/{thumb,display,original,meta}` in `PHOTO_BUCKET`; `/api/photos/upload` writes per-item metadata + `photography/meta/index.json`, and admin drag/repack writes `photography/meta/layout.json` via `/api/photos/layout`.
