...
- Photography uploads now support a multi-variant pipeline (`thumb.webp`, `display.webp`, `original`) written under `photography/*` keys, with per-item meta JSON and global `index.json`/`layout.json` files in `photography/meta/`.
- Admin layout persistence is available via `/admin/photography/` + `POST /api/photos/layout`; public photography pages still use mosaic drag/repack interactions but only admin mode writes layout metadata.
