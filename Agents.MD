...
- JavaScript fetches YouTube playlist data from the YouTube Data API for each playlist.
- Fetching goes through a local **Pages Function** proxy (`/api/youtube-playlist`) so the browser never sees API keys.
- The Pages Function reads `YT_API_KEY` from environment variables in Cloudflare Pages.
