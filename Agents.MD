...
- Media pages live under `/pages/spincline/`, `/pages/photography/`, and `/pages/admin/upload/`.
- API routes live under `/functions/api/**` (Cloudflare Pages Functions pattern).
- D1 (`db/schema.sql`) is the source of truth for media metadata/order/visibility; raw files are in R2.
- Admin uploads are direct browser-to-R2:
  - Images: presigned PUT (`/api/admin/image/init` + `/api/admin/image/complete`)
  - Videos: resumable multipart (`/api/admin/multipart/*`)
- Security model: Cloudflare Access protects `/admin/*` and `/api/admin/*`; optional `ADMIN_TOKEN` check is supported server-side when configured.
- Keep README in sync whenever media APIs, schema, or upload behavior changes.
