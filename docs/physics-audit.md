...
## Follow-up: boundary bounce simplification

- Observed issue: quick perceived "double bounce" when motion hit lower boundaries.
- Change: in `SitePhysics.animateScrollTo`, boundary velocity inversion now happens only on the first boundary impact for a given excursion; subsequent constrained frames apply damping without repeated sign flips.
- Result: a single damped rebound with faster visual settling and less jitter/chatter near the lower edge.
