...
## ScrollShockAbsorber deep audit refresh

- Previous model mixed spring dynamics with an optional release kick, which could produce asymmetric visual artifacts at bottom edge under repeated wheel/touch input.
- Updated absorber architecture to a cleaner force-first model:
  - near-critically damped spring computed from `stiffness`, `mass`, and `dampingRatio`
  - adaptive stiffness by displacement for firm but controlled boundary compression
  - velocity-continuous semi-implicit integration and jerk limiting
  - edge-lock while displaced to prevent top/bottom ping-pong capture glitches
  - native scrolling untouched unless boundary intent is explicit
- `/pages/scroll-shock/` demo was updated to reflect current parameters and tuning guidance.
